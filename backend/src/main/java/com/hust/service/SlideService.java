...
public class SlideService {
    ...
    // --- 1. Tạo Slide Nhanh (Quick Creation) ---
    @Transactional
    public PresentationResponse createQuickSlide(QuickCreateRequest request, Long currentUserId) {
        ...
        // - Content: "<title line>\n<body> --- <title line>\n<body>" => tách block content theo slide.
        List<String> titleBlocks = splitSlideBlocks(request.getTitle(), true);
        List<String> contentBlocks = splitSlideBlocks(request.getContent(), false);
        ...
        if (request.getTemplateId() != null) {
            ...
            // Format đề xuất cho mỗi block: dòng đầu là title, các dòng sau là body (có thể structured với "\\--"/"\\-").
            // Nếu không tách, giữ logic cũ: structured -> dùng chung; unstructured -> chia đoạn theo số text box.
            ...
            String presentationTitle = (hasPerSlideTitles && !titleBlocks.isEmpty()) ? titleBlocks.get(0) : request.getTitle();
            ...
            for (int i = 0; i < templateSlides.size(); i++) {
                ...
                QuickCreateRequest perSlideReq = cloneQuickCreateRequestWithTitleAndContent(request, perSlideTitle, slideContent);
        ...
        for (int i = 0; i < slideCount; i++) {
            ...
            if (!hasPerSlideContents && slideCount > 1 && !legacyStructuredMode) {
                // Khi user không tách content theo slide, chia đoạn theo số ô text/caption của từng slide layout.
            ...
            QuickCreateRequest perSlideReq = cloneQuickCreateRequestWithTitleAndContent(request, perSlideTitle, perSlideContent);

            Slide slide = new Slide();
            slide.setPresentation(savedPresentation);
            slide.setSlideIndex(i + 1);

            if ((hasRequestLayoutJsons || hasRequestLayout) && layoutJsonForSlide != null && !layoutJsonForSlide.trim().isEmpty()) {
        ...
        // historyLogService.logAction("QUICK_CREATE", "PRESENTATION", savedPresentation.getId(), currentUserId);
    ...
    private int countTextBoxesInLayoutJson(String layoutJson) {
        if (layoutJson == null || layoutJson.isBlank()) return 0;
        ...
        for (Map<String, Object> e : els) {
            String type = String.valueOf(e.getOrDefault("type", ""));
            if ("text".equalsIgnoreCase(type) || "caption".equalsIgnoreCase(type)) count++;
        }
        return count;
    }
    ...
    private QuickCreateRequest cloneQuickCreateRequestWithTitleAndContent(QuickCreateRequest src, String title, String content) {
    ...
    private List<Map<String, Object>> extractElementsFromLayoutJson(String layoutJson) {
        if (layoutJson == null || layoutJson.trim().isEmpty()) return List.of();
        try {
    ...
    public Slide addNewSlideToPresentation(Long projectId, Long currentUserId) {
        ...
        long currentSlideCount = slideRepository.countByPresentationIdOrderBySlideIndexAsc(projectId);
        int newIndex = (int) currentSlideCount + 1;
        
        // Lấy Layout Template Mặc định
        TemplateSlide defaultLayout = templateSlideRepository.findById(1L)
                .orElseGet(() -> {
                    TemplateSlide fallback = new TemplateSlide();
                    fallback.setLayoutJson("{\"elements\": []}");
                    return fallback;
        ...
        // Cập nhật ngày chỉnh sửa của Presentation cha
        presentation.setEditedDate(Instant.now());
        presentationRepository.save(presentation);
        
        // historyLogService.logAction("ADD_SLIDE", "SLIDE", savedSlide.getId(), currentUserId);
        return savedSlide;
    }
    ...
    public Slide updateSlideContent(Long slideId, SlideUpdateRequest request, Long currentUserId) {
        ...
        if (request.getUpdatedContentJson() != null && !request.getUpdatedContentJson().trim().isEmpty()) {
            ...
            // Ngược lại chỉ cập nhật title/content vào JSON hiện có (hỗ trợ cả legacy và dạng {layout/template,data}).
            String newContentJson = updateJsonWithTitleAndContent(slide.getContentJson(), request.getTitle(), request.getContent());
        ...
        if ("PPTX".equals(format)) {
            try (org.apache.poi.xslf.usermodel.XMLSlideShow ppt = new org.apache.poi.xslf.usermodel.XMLSlideShow()) {
                ...
                for (Slide slide : slides) {
                    ...
                    // Nếu slide có layout đã lưu (từ editor) thì render theo danh sách elements trong JSON.
                    ...
                    for (Map<String, Object> el : elements) {
                        ...
                        if ("title".equals(type)) {
                            ...
                            if (structuredMode && structured != null) {
                                java.util.List<String> list = "caption".equals(type) ? structured.captions : structured.texts;
            ...
            for (Slide slide : slides) {
                ...
                try {
                    ...
                    if (elements.isEmpty()) {
                        ...
                        elements = List.of(
                                Map.of("id", 1, "type", "title", "x", 40, "y", 40, "w", 720, "h", 90,
                                        "style", Map.of("fontSize", 40, "bold", true, "italic", false, "underline", false, "align", "left", "color", "#111827", "fontFamily", fontFamily)),
                                Map.of("id", 2, "type", "text", "x", 40, "y", 150, "w", 720, "h", 360,
                                        "style", Map.of("fontSize", 18, "bold", false, "italic", false, "underline", false, "align", "left", "color", "#111827", "fontFamily", fontFamily))
                        );
                    }
                    ...
                    for (Map<String, Object> el : ordered) {
                        ...
                        Map<String, Object> style = (el.get("style") instanceof Map<?, ?> m) ? (Map<String, Object>) m : Map.of();
    ...
    private byte[] generatePdfExport(Presentation presentation, List<Slide> slides, String fontFamily) throws IOException {
    ...
    private void renderElementToGraphics(Graphics2D g, int x, int y, int w, int h, String text, Map<String, Object> style, String defaultFontFamily) {
        ...
        Font font = new Font(family == null || family.isBlank() ? defaultFontFamily : family, awtStyle, Math.max(6, fontSize));
    ...
    private void renderElementToPdf(PDPageContentStream cs, PDFont font, float x, float yTop, float w, float h, String text, Map<String, Object> style, float pageHeight) throws IOException {
        String align = asString(style.getOrDefault("align", "left")).toLowerCase(Locale.ROOT);
    ...
    private java.util.List<String> wrapTextPdf(String text, PDFont font, float fontSize, float maxWidth) throws IOException {
        String normalized = text == null ? "" : text.replace("\r\n", "\n");
        java.util.List<String> out = new java.util.ArrayList<>();
        for (String para : normalized.split("\n")) {
            String p = para == null ? "" : para;
    ...
    private Color parseColor(String raw) {
        if (raw == null) return new Color(17, 24, 39);
        String s = raw.trim();
    ...
    private PDFont loadBundledPdfFont(PDDocument doc, String... resourcePaths) {
        ...
        for (String res : resourcePaths) {
            if (res == null || res.isBlank()) continue;
     ...
     * Lưu ý: hàm này dùng {@link String#format} để ghép chuỗi trực tiếp, vì vậy chỉ nên xem như phương án legacy/fallback.
     */
     ...
     * {@code layoutJson} được nhúng trực tiếp vào JSON nên cần là một JSON hợp lệ (object) từ template.
     */
     ...
     * - Nếu không có {@code data}, fallback sang dạng legacy (title/content ở root).
     * - Nếu parse JSON thất bại, fallback cuối cùng là thay chuỗi theo kiểu mock cũ.
     */
    ...
    private Map<String, Object> extractDataMap(String contentJson) {
        if (contentJson == null || contentJson.trim().isEmpty()) return Map.of();
        try {
    ...
    private List<Map<String, Object>> extractElements(String contentJson) {
        if (contentJson == null || contentJson.trim().isEmpty()) return List.of();
        try {
            Map<String, Object> root = objectMapper.readValue(contentJson, Map.class);
    ...
    private static class StructuredContent {
        ...
        StructuredContent(java.util.List<String> images, java.util.List<String> captions, java.util.List<String> texts, java.util.List<String> dates) {
            this.images = images;
        ...
        /** Lấy giá trị theo `slotIndex` (bắt đầu từ 1) trong `list`; nếu thiếu thì trả về chuỗi rỗng. */
    ...
    /** Nhận biết `content` có dùng token phân tách cấu trúc ("\\-" hoặc "\\--") hay không. */
    ...
    /**
        ...
        * Nếu có token, giữ cả phần rỗng để hỗ trợ “bỏ qua slot #1 nhưng vẫn điền slot #2”.
     */
    private java.util.List<String> splitItems(String section) {
        ...
        if (section.contains("\\-")) {
            ...
            java.util.ArrayList<String> out = new java.util.ArrayList<>();
            for (String p : parts) out.add(p == null ? "" : p.trim());
            return out;
        }
    ...
    private java.util.Map<Integer, Integer> buildSlotIndexByElementId(java.util.List<java.util.Map<String, Object>> elements) {
    ...
    private void applyParagraphAlign(org.apache.poi.xslf.usermodel.XSLFTextParagraph p, Map<String, Object> el) {
        try {
            ...
            if (!(styleObj instanceof Map<?, ?> style)) return;
    ...
    private void applyRunStyle(org.apache.poi.xslf.usermodel.XSLFTextRun r, Map<String, Object> el, String defaultFontFamily) {
        try {
            Object styleObj = el.get("style");
            ...
            if (ff == null || ff.isBlank()) ff = defaultFontFamily;
            r.setFontFamily(ff);
