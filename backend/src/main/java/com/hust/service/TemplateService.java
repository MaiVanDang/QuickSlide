...
public class TemplateService {
    ...
    // @Autowired private HistoryLogService historyLogService; // Giả định
    // HistoryLogService

    // --- 1. Xem Thư viện (No. 4) ---
    @Transactional(readOnly = true)
    public List<TemplateResponse> getTemplates(String type, Integer currentUserId) {
    ...
    @Transactional
    public TemplateResponse createNewTemplate(TemplateCreateRequest request, Integer currentUserId) {
        ...
        template.setTheme(
                request.getTheme() == null || request.getTheme().isBlank() ? "default" : request.getTheme().trim());
        ...
        // historyLogService.logAction("CREATE_TEMPLATE", "TEMPLATE",
        // savedTemplate.getId(), currentUserId);
        return toTemplateResponse(savedTemplate, currentUserId);
    }

    // --- 2b. Lấy 1 template (để sửa) ---
    @Transactional(readOnly = true)
    public TemplateResponse getTemplateById(Integer templateId, Integer currentUserId) {
    ...
    @Transactional
    public TemplateResponse updateTemplate(Integer templateId, TemplateCreateRequest request, Integer currentUserId) {
        if (currentUserId == null) {
            throw new com.hust.exception.UnauthorizedException("Chưa đăng nhập");
        }
    ...
    @Transactional
    public void softDeleteTemplate(Integer templateId, Integer currentUserId) {
        Template template = templateRepository.findById(templateId)
                .orElseThrow(() -> new ResourceNotFoundException("Template không tồn tại."));

        // Kiểm tra quyền: Chỉ Owner mới được xóa mềm
        if (!template.getOwner().getId().equals(currentUserId)) {
            // Ném SecurityException (sẽ được GlobalExceptionHandler bắt và trả về 400 Bad
            // Request)
        ...
        // historyLogService.logAction("SOFT_DELETE_TEMPLATE", "TEMPLATE", templateId,
        // currentUserId);
    ...
    @Transactional(readOnly = true)
    public List<com.hust.dto.response.TemplateSlideResponse> getTemplateSlides(Integer templateId,
    ...
    // --- Helper Mapper ---
    private TemplateResponse toTemplateResponse(Template t, Integer currentUserId) {
        ...
        // Tương thích ngược: template cũ chưa có preview hoặc preview SVG placeholder
        // đời cũ
