...
public class HelpService {
    ...
    private final List<HelpTopicResponse> mockData = Arrays.asList(
            HelpTopicResponse.builder().id(1).title("QuickSlideへようこそ").preview("Hướng dẫn cơ bản")
                    .category("getting-started").build(),
            HelpTopicResponse.builder().id(2).title("最初のスライドを作成する").preview("Tạo slide đầu tiên")
                    .category("getting-started").build(),
            HelpTopicResponse.builder().id(3).title("テンプレートの使い方").preview("Sử dụng Template")
                    .category("getting-started").build(),

            HelpTopicResponse.builder().id(4).title("アカウントの作成").preview("Đăng ký tài khoản mới").category("account")
                    .build(),
            HelpTopicResponse.builder().id(5).title("パスワードのリセット").preview("Khôi phục mật khẩu").category("account")
                    .build(),

            HelpTopicResponse.builder().id(6).title("クイック作成の使い方").preview("Sử dụng Quick Creation").category("guide")
                    .build(),
            HelpTopicResponse.builder().id(7).title("バッチ生成の活用").preview("Tạo slide hàng loạt").category("guide")
                    .build());
